...
import { BranchPersonnelManagement } from "@/components/branches/branch-personnel-management";
...
export function BranchManagement() {
  ...
  const [personnelBranch, setPersonnelBranch] = useState<Branch | null>(null);
  ...
  const canManagePersonnel = user?.role === "CRM_OWNER" || user?.role === "ORGANIZATION_OWNER" || user?.role === "BRANCH_ADMIN";
  ...
  if (personnelBranch && user && token && canManagePersonnel) {
    return <BranchPersonnelManagement branch={personnelBranch} key={personnelBranch.id} onClose={() => setPersonnelBranch(null)} organizationName={selectedOrganization?.name || `Organization #${personnelBranch.organizationId}`} token={token} user={user} />;
  }
  ...
  return (
    <div className="grid gap-[var(--section-gap)]">
      ...
      <Dialog className="max-w-lg" description="Sanitized Branch information and audit details." isOpen={Boolean(detail)} onClose={() => setDetail(null)} title="Branch details">
        {detail ? <BranchDetails branch={detail} canEdit={canEditBranch(user, detail)} canManage={isManager} canManagePersonnel={Boolean(canManagePersonnel)} isLoading={isDetailLoading} organizationName={isCrmOwner ? selectedOrganization?.name || "Not available" : undefined} onChangeStatus={setBranchToChange} onEdit={startEdit} onManageAdmins={setAdminBranch} onManagePersonnel={setPersonnelBranch} /> : null}
      </Dialog>
...
function BranchRow({ branch, canEdit, canManage, canManagePersonnel, organizationName, onChangeStatus, onEdit, onManageAdmins, onManagePersonnel, onView }: { branch: Branch; canEdit: boolean; canManage: boolean; canManagePersonnel: boolean; organizationName?: string; onChangeStatus: (branch: Branch) => void; onEdit: (branch: Branch) => void; onManageAdmins: (branch: Branch) => void; onManagePersonnel: (branch: Branch) => void; onView: (branch: Branch) => void }) {
  return <tr className="hover:bg-[var(--color-surface-hover)]"><td className="px-5 py-4"><button className="flex items-center gap-3 text-left" onClick={() => void onView(branch)} type="button"><span className="grid size-9 place-items-center rounded-full bg-[var(--blue-100)] text-[var(--color-primary)]"><MapPin className="size-[var(--icon-sm)]" /></span><span className="font-semibold text-[var(--color-text)] hover:text-[var(--color-primary)]">{branch.name}</span></button></td><td className="px-5 py-4 text-[var(--color-text-secondary)]">{branch.address}</td><td className="px-5 py-4 text-[var(--color-text-secondary)]">{branch.phone || "Not provided"}</td>{organizationName ? <td className="px-5 py-4 text-[var(--color-text-secondary)]">{organizationName}</td> : null}<td className="px-5 py-4"><BranchStatusPill status={branch.status} /></td><td className="px-5 py-4 text-[var(--color-text-secondary)]">{formatDate(branch.updatedAt)}</td><td className="px-5 py-4"><div className="flex justify-end gap-2"><Button aria-label={`View ${branch.name}`} onClick={() => void onView(branch)} variant="ghost"><Eye className="size-[var(--icon-sm)]" /></Button>{canManagePersonnel ? <Button className="gap-1" onClick={() => onManagePersonnel(branch)} variant="secondary">Manage Personnel</Button> : null}{canManage ? <Button className="gap-1" onClick={() => onManageAdmins(branch)} variant="secondary">Manage Admins</Button> : null}{canEdit ? <Button aria-label={`Edit ${branch.name}`} className="gap-1" onClick={() => onEdit(branch)} variant="secondary"><Pencil className="size-3.5" />Edit</Button> : null}{canManage ? <Button aria-label={`${branch.status === "ACTIVE" ? "Deactivate" : "Reactivate"} ${branch.name}`} className="gap-1" onClick={() => onChangeStatus(branch)} variant="secondary">{branch.status === "ACTIVE" ? <ToggleLeft className="size-3.5" /> : <ToggleRight className="size-3.5" />}{branch.status === "ACTIVE" ? "Deactivate" : "Reactivate"}</Button> : null}</div></td></tr>;
}

function BranchDetails({ branch, canEdit, canManage, canManagePersonnel, isLoading, organizationName, onChangeStatus, onEdit, onManageAdmins, onManagePersonnel }: { branch: Branch; canEdit: boolean; canManage: boolean; canManagePersonnel: boolean; isLoading: boolean; organizationName?: string; onChangeStatus: (branch: Branch) => void; onEdit: (branch: Branch) => void; onManageAdmins: (branch: Branch) => void; onManagePersonnel: (branch: Branch) => void }) {
  return <div className="grid gap-4 text-sm"><div><p className="text-lg font-bold text-[var(--color-text)]">{branch.name}</p><div className="mt-2"><BranchStatusPill status={branch.status} /></div></div>{isLoading ? <p className="text-[var(--color-text-secondary)]">Refreshing details...</p> : null}<DetailRow label="Address" value={branch.address} /><DetailRow label="Phone" value={branch.phone || "Not provided"} />{organizationName ? <DetailRow label="Organization" value={organizationName} /> : null}<DetailRow label="Created by" value={actorSummary(branch.createdByUserId)} /><DetailRow label="Created" value={formatDate(branch.createdAt)} /><DetailRow label="Last updated" value={formatDate(branch.updatedAt)} />{branch.deactivatedAt ? <><DetailRow label="Deactivated by" value={actorSummary(branch.deactivatedByUserId)} /><DetailRow label="Deactivated" value={formatDate(branch.deactivatedAt)} /></> : null}<div className="mt-2 flex flex-wrap justify-end gap-3">{canManagePersonnel ? <Button onClick={() => onManagePersonnel(branch)} variant="secondary">Manage Personnel</Button> : null}{canManage ? <Button onClick={() => onManageAdmins(branch)} variant="secondary">Manage Admins</Button> : null}{canEdit ? <Button onClick={() => onEdit(branch)} variant="secondary">Edit</Button> : null}{canManage ? <Button onClick={() => onChangeStatus(branch)} variant="secondary">{branch.status === "ACTIVE" ? "Deactivate" : "Reactivate"}</Button> : null}</div></div>;
}
